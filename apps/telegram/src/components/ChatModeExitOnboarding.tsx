...
const STORAGE_KEY = 'has_seen_chat_exit_onboarding_v1'

export function ChatModeExitOnboarding() {
    ...
    useEffect(() => {
        ...
        const checkTarget = () => {
            const element = document.getElementById('studio-mode-toggle')
            if (element) {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] font-sans">
            ...
            >
                ...
                <div className="absolute inset-0 rounded-lg border-2 border-cyan-500" />
            </div>

            {/* Arrow */}
            <div
                className="absolute z-[110] text-cyan-500 pointer-events-none animate-bounce"
                style={{
            ...
            >
                <div className="w-[300px] bg-zinc-900 border border-cyan-500/30 rounded-2xl p-5 shadow-2xl relative animate-in fade-in slide-in-from-top-4 duration-500">
                    <button
                        onClick={handleClose}
                    ...
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <h3 className="font-bold text-white text-lg">Режим Студии</h3>
                    </div>

                    <p className="text-white/80 text-sm leading-relaxed mb-4">
                        Нажмите на <b>Studio</b>, чтобы вернуться к созданию изображений и видео.
                    </p>

                    <div className="flex justify-end">
                        <button
                            onClick={handleClose}
                            className="px-5 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white font-medium rounded-xl text-sm transition-colors flex items-center gap-2"
                        >
                            <span>Понятно</span>
